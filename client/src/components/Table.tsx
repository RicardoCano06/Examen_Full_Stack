      ...
      <table className="min-w-full divide-y divide-slate-100">
        <thead>
          <tr className="border-b border-slate-300 bg-slate-200">
            {headers.map((h) => (
              <th key={labelOf(h)} scope="col" className={`px-4 py-2 text-xs font-semibold uppercase tracking-wider text-slate-700${centered(h) ? ' text-center' : ' text-left'}`}>{labelOf(h)}</th>
            ))}
          </tr>
        </thead>
